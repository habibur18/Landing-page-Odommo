  ...
  return (
    <div className="text-center my-10">
      {genres.map((genre) => (
        ...
        //   {genre.name}
        // </button>
        <button key={genre.id} className={`hover:underline underline-offset-4 duration-300 font-semibold px-5 py-3 rounded-full mx-2 my-3`} onClick={() => handleFilter(genre.id)}>
          {genre.name}
        </button>
      ))}
    </div>
